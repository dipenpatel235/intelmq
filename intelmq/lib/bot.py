...
class Bot(object):
    ...
    def __init__(self, bot_id: str, start: bool = False, sighup_event=None,
        ...
        try:
            ...
            broker = getattr(self, "source_pipeline_broker",
                             getattr(self, "broker", "redis")).title()
            if broker != 'Amqp':
                self.is_multithreadable = False

            """ Multithreading """
            if (getattr(self, 'instances_threads', 0) > 1 and
        ...
        self.__stats_cache = cache.Cache(host=getattr(self,
                                                      "statistics_host",
                                                      "127.0.0.1"),
                                         port=getattr(self,
                                                      "statistics_port", "6379"),
    ...
    def __connect_pipelines(self):
        if self.__source_queues:
            ...
            #TODO: pass the relevant variables instead of parameters
